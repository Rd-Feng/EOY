...
router.get('/subcomments', (req, res) => {
  connection.query('SELECT * FROM subcomments ORDER BY created_at DESC', (err, results) => {
    if (err) {
      console.log(err);
...
router.get('/subcomments/:comment_id', (req, res) => {
  connection.query('SELECT * FROM subcomments WHERE comment_id=' + connection.escape(req.params.comment_id) + ' ORDER BY created_at DESC', (err, results) => {
    if (err) {
      console.log(err);
      res.send({ status: 'failed' });
    } else {
